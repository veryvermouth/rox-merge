...
- **외부(디스크) 변경 감지**: 창 활성화·탭 전환 시 좌/우 파일이 디스크에서 바뀌었는지(mtime·size) 확인해 모달로 알리고, 동의하면 다시 로드(“무시”는 다음 변경까지 유지). 보기 메뉴 **“외부 변경 감지”** 토글로 끌 수 있음. `F5`로 언제든 디스크에서 다시 로드(미저장 편집이 있으면 확인 후).
...
- **경로 바**: 좌/우 폴더 경로 입력칸 + `...` 찾아보기 + 포커스 시 **최근 폴더 자동완성 드롭다운**.
- `F5` 새로고침 시 열려 있던 하단 diff도 닫고 트리만 다시 비교.

### 공통
- **드래그 & 드롭**: 파일 비교 창엔 파일을, 폴더 비교 창엔 폴더를 끌어다 놓아 열기(2개 → 좌/우, 1개 → 놓은 위치 기준).
- **빈 화면 웰컴**: 모든 탭을 닫으면 “ROX-MERGE” 로고 화면을 표시.
- **비교 옵션**: 공백 무시 / 대소문자 무시 / 이동 탐지 (파일), 정확 비교 / 다른 항목만 / 새 탭 모드 (폴더).
- **테마**: 라이트/다크 전역 토글(Fusion 스타일로 메뉴바·툴바·트리까지 함께 전환, OS 테마 무관).
- **글꼴 크기**: 파일 diff 글꼴(파일 탭·폴더 하단 diff 공유)과 폴더 트리 글꼴을 **따로 관리**(폴더 탭은 diff↔트리 포커스에 따라 확대/축소·리셋 대상이 갈림). 폴더 트리는 **맑은 고딕**으로 표시.
- **설정 저장/복원(QSettings)**: 글꼴 크기, 테마, 비교 옵션, 최근 파일·폴더 목록, 외부 변경 감지 여부.
- **큰 파일 가드**: 아주 큰 파일은 이동 탐지 자동 비활성 / diff 생략(플레인)으로 안정성 확보.
- **도움말 › 단축키(F1)**: 현재 단축키·설명을 카테고리별로 보여주는 창.
...
| `F5` | (파일) 디스크에서 다시 로드(미저장 편집 시 확인) · (폴더) 새로고침(+하단 diff 닫기) |
| `F1` | 단축키 도움말 창 |
